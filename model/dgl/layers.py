...
class RGCNBasisLayer(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim, num_rels, num_bases=-1, bias=None,
        ...
        # optional structural prior (relation-role tfidf)
        self.tfidf = None

    def set_tfidf(self, tfidf_tensor):
        """
        tfidf_tensor: shape [num_relations, num_roles]
        """
        self.tfidf = tfidf_tensor
    ...
    def propagate(self, g, attn_rel_emb=None):
        ...
        def msg_func(edges):
            ...
            # structural TF-IDF weighting (optional)
            if self.tfidf is not None and 'role' in edges.dst:
                rel = edges.data['type']
                role = edges.dst['role'].long()
                structural_w = self.tfidf[rel, role].unsqueeze(1)
                msg = msg * structural_w
