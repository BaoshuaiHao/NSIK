...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None,
                 add_traspose_rels=False, num_neg_samples_per_link=1, use_kge_embeddings=False,
                 dataset='', kge_model='', file_name='', use_global_emb=True, use_node_roles=True):
        ...
        # ===== DEBUG: resolve dataset base path =====
        print("\n===== DATASET DEBUG =====")
        print("raw_data_paths:", raw_data_paths)

        base_path = None
        if isinstance(raw_data_paths, dict) and len(raw_data_paths) > 0:
            values = list(raw_data_paths.values())
            print("raw_data_paths values:", values)
            base_path = os.path.dirname(values[0])

        print("resolved base_path:", base_path)

        # load global diffusion embedding (optional)
        self.global_emb = None
        if use_global_emb and base_path is not None:
            global_path = os.path.join(base_path, "global_emb.npy")
            print("expect global_emb:", global_path)
            if os.path.exists(global_path):
                logging.info(f"Loading global diffusion embedding from {global_path}")
                self.global_emb = np.load(global_path)
            else:
                print("global_emb.npy NOT FOUND")
        elif not use_global_emb:
            print("global_emb disabled by params")

        # load structural role ids (optional)
        self.role_ids = None
        if use_node_roles and base_path is not None:
            role_path = os.path.join(base_path, "node_role_ids.npy")
            print("expect node_role_ids:", role_path)
            if os.path.exists(role_path):
                logging.info(f"Loading node role ids from {role_path}")
                self.role_ids = np.load(role_path)
            else:
                print("node_role_ids.npy NOT FOUND")
        elif not use_node_roles:
            print("node_role_ids disabled by params")
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        ...
        # append global diffusion embedding
        if self.global_emb is not None:
            global_feats = self.global_emb[nodes]
            if n_feats is not None:
                n_feats = np.concatenate([n_feats, global_feats], axis=1)
            else:
                n_feats = global_feats

        # attach structural role ids to nodes (for TF-IDF weighted propagation)
        if self.role_ids is not None:
            subgraph.ndata['role'] = torch.LongTensor(self.role_ids[nodes])
