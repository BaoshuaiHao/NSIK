    ...
    for link in all_links:
        ...
        if node_features is not None:
            if kge_entity2id is not None:
                kge_nodes = np.array(
                    [kge_entity2id[id2entity[n]] for n in nodes],
                    dtype=np.int64
                )
                n_feats = node_features[kge_nodes]
            else:
                # global_emb case: indices already match entity ids
                n_feats = node_features[np.array(nodes, dtype=np.int64)]
        else:
            n_feats = None
...
def main(params):
    ...
    # ===== load global node embeddings (custom prior features) =====
    global_emb_path = os.path.join('./data', params.dataset, 'global_emb.npy')
    if not params.disable_global_emb and os.path.exists(global_emb_path):
        print(f"Loading global embeddings from: {global_emb_path}")
        node_features = np.load(global_emb_path)
        # sanity check: must be 2D [num_nodes, dim]
        if node_features.ndim != 2:
            raise ValueError(f"global_emb.npy must be 2D, got shape {node_features.shape}")
        kge_entity2id = None
    else:
        # fallback to original KGE embeddings logic
...
if __name__ == '__main__':
    ...
    parser.add_argument('--disable_global_emb', action='store_true',
                        help='Disable loading global_emb.npy as node features')
